...
report$WT_D9_vs_KD_D9.GO <- 
  string_db$get_enrichment(subset(RNAseq2, exp == 'WT D9' & 
                                    ! is.na(log2FoldChange) &  
                                    ! is.na(STRING_id) & 
                                    padj <= 1e-5 & 
                                    abs(log2FoldChange) >= 2 &
...
report$TrpM_D9_vs_KD_D9.GO <- 
  string_db$get_enrichment(subset(RNAseq2, exp == 'TrpM D9' & 
                                    ! is.na(log2FoldChange) &  
                                    ! is.na(STRING_id) & 
                                    padj <= 1e-5 & 
                                    abs(log2FoldChange) >= 2 &
...
# Retreive intSite data from database.
report$intSites <- getIntSiteData('specimen_management', 
                           'intsites_miseq', 
                           patients = unique(samples$Patient))


# Standardize break points by sample.
cluster <- makeCluster(report$CPUs)
report$intSites <- unlist(
  GRangesList(
    parLapply(cluster, split(report$intSites, report$intSites$sampleName), 
              function(p){
...
report$intSites <- unlist(
  GRangesList(
    parLapply(cluster, split(report$intSites, report$intSites$patient), 
              function(p){
                library(GenomicRanges)
                library(gintools)
...
report$intSites <- gt23::calcReplicateAbundances(report$intSites)

# Create a splitting vector for parallelization.
report$intSites$s <- ntile(1:length(report$intSites), report$CPUs)

# Add nearest gene and nearest oncogene annotations.
names(report$intSites) <- NULL
report$intSites  <- unlist(
  GRangesList(
    parLapply(cluster, split(report$intSites, report$intSites$s), 
    #lapply(split(report$intSites, report$intSites$s), 
              function(p){
                library(gt23)
                
                p <- gt23::nearestGenomicFeature(p, subject = gt23::hg38.refSeqGenesGRanges, subject.exons = gt23::hg17.refSeqGenesGRanges.exons)
                p$order <- 1:length(p)
                
                o <- gt23::nearestGenomicFeature(p, subject = gt23::hg38.refSeqGenesGRanges, subject.exons = gt23::hg17.refSeqGenesGRanges.exons, geneList = gt23::humanOncoGenesList)
                o <- o[order(o$order)]
                p$nearestOncoFeature       <- o$nearestFeature
                p$nearestOncoFeatureDist   <- o$nearestFeatureDist
                p$nearestOncoFeatureStrand <- o$nearestFeatureStrand
                
                o <- gt23::nearestGenomicFeature(p, subject = gt23::hg38.refSeqGenesGRanges, subject.exons = gt23::hg17.refSeqGenesGRanges.exons, geneList = gt23::humanLymphomaGenesList)
                o <- o[order(o$order)]
                p$nearestlymphomaFeature       <- o$nearestFeature
                p$nearestlymphomaFeatureDist   <- o$nearestFeatureDist
                p$nearestlymphomaFeatureStrand <- o$nearestFeatureStrand
...
# Create a plot of binned abundances, separated by genotype, for clones with abundances >= 50
# to compliment the longitudinal relative abuncance plot.
report$maxClonalAbundances_gte_50_abund <- 
  dplyr::group_by(data.frame(report$intSites), patient, posid) %>%
  dplyr::filter(timePointDays == max(timePointDays)) %>%
  dplyr::summarise(maxAbund = max(estAbund)) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(bin  = cut(maxAbund, breaks = seq(from=0, to=max(maxAbund)+10, by = 10), right = TRUE, dig.lab=10)) %>%
  dplyr::mutate(bin2 = cut(maxAbund, breaks = seq(from=0, to=max(maxAbund)+10, by = 10), right = TRUE, label = FALSE, dig.lab=10)) %>%
  dplyr::mutate(patientType = ifelse(grepl('Y664F', patient), 'Y664F', 'WT')) %>%
  dplyr::group_by(patientType, bin2) %>%
  dplyr::summarise(binLabel = bin[1], binCount = n()) %>%
  dplyr::ungroup() %>%
  dplyr::filter(bin2 >= 6) %>%
  {
    ggplot(., aes(x = binLabel, y = binCount)) +
    theme_bw()+
    geom_bar(stat='identity') +
    labs(x = 'Clonal abundance bins', y = 'Clones') + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    theme(axis.text=element_text(size=12), axis.title=element_text(size=16)) +
    scale_x_discrete(breaks = unique(as.character(sort(.$binLabel)))[c(T,F,F)]) +
    facet_grid(patientType~.)
  }


report$clonesAbund_gte_200 <- dplyr::group_by(data.frame(report$intSites), patient, posid) %>%
  dplyr::filter(timePointDays == max(timePointDays)) %>%
  dplyr::summarise(estAbund = max(estAbund),
                   nearestFeature = nearestFeature[1],
                   nearestFeatureDist = nearestFeatureDist[1],
                   STRING_id = STRING_id[1]) %>%
  dplyr::ungroup() %>%
  dplyr::filter(estAbund >= 200) 


report$clonesAbund_gte_200.2 <- dplyr::group_by(data.frame(report$intSites), patient, posid) %>%
  dplyr::summarise(estAbund = max(estAbund),
                   nearestFeature = nearestFeature[1],
                   nearestFeatureDist = nearestFeatureDist[1],
                   STRING_id = STRING_id[1]) %>%
...
# Considering all intSites (WT and Y664F clones) with abundances >= 200, identify enriched KEGG terms 
report$MostAbundantCloneKEGG <- string_db$get_enrichment(subset(report$intSites, estAbund >= 200)$STRING_id, category = 'KEGG', methodMT = "fdr", iea = FALSE)
report$MostAbundantCloneKEGG <- subset(report$MostAbundantCloneKEGG, pvalue_fdr <= 0.05)
report$MostAbundantCloneKEGG$RNAseq_top_term <- report$MostAbundantCloneKEGG$term_id %in% report$WT_D9_vs_KD_D9.KEGG$term_id


# When looking at the top 200 most abundand clones, most were withing RefSeq annotated genes but a number were not.
# Reviewing these sites in the UCSD browser showed that many sites not in RefSeq genes are within RNA or uncharacterized 
# genes. Here we updated the automated annoations and mark them with a asterisk. 
oncogenes <- c(gt23::humanOncoGenesList, 'TET2,TET2-AS1')
...
report$highAbundClones_200 <- 
  report$clonesAbund_gte_200 %>%
  dplyr::select(nearestFeature, nearestFeatureDist, estAbund, patient) %>%
  dplyr::mutate('Subject' = ifelse(grepl('Y664F', patient), 'Y664F', 'WT')) %>%
  dplyr::mutate('Oncogene' = ifelse(toupper(nearestFeature) %in% toupper(oncogenes), 'Yes', 'No')) %>%
  dplyr::select(-patient) %>%
  dplyr::arrange(desc(estAbund)) %>%
  dplyr::rename(gene = nearestFeature) %>%
  dplyr::rename(dist = nearestFeatureDist) %>%
  dplyr::mutate(n = 1:n()) %>%
  dplyr::group_by(gene) %>%
  dplyr::summarise(dist = paste(dist, collapse = ','),
                   estAbund = paste(estAbund, collapse = ','),
 ...
 o <- table(unique(report$clonesAbund_gte_200$STRING_id[! is.na(report$clonesAbund_gte_200$STRING_id)]) %in% string_db$get_term_proteins(report$WT_D9_vs_KD_D9.KEGG$term_id)$STRING_id)
 sprintf("%.2f%%", (o[which(names(o) == 'TRUE')] / sum(o))*100)
...
o1 <- subset(report$d, experiment == 'WT_D9_vs_KD_D9' &  ! is.na(log2FoldChange) &  ! is.na(STRING_id) & padj <= 0.01 & abs(log2FoldChange) >= 4) # 720 STRING ids
 # 682 STRING ids

e1 <- string_db$get_enrichment(o1$STRING_id, category = 'KEGG', methodMT = "fdr", iea = FALSE)
e1 <- subset(e1, pvalue_fdr <= 0.05)

e2 <- string_db$get_enrichment(o2$STRING_id, category = 'KEGG', methodMT = "fdr", iea = FALSE)
e2 <- subset(e2, pvalue_fdr <= 0.05)

e2[e2$term_id %in% e1$term_id,]


t <- table(e1$term_id %in% e2$term_id)
report$percent.KEGG.overlap.intSites.RNAseq <- sprintf("%.2f%%", (t[which(names(t) == 'TRUE')] / n_distinct(e$term_id)*100))
...
d.wt <- dplyr::group_by(data.frame(subset(report$intsites.WT, estAbund >= 50)), posid) %>%
  dplyr::filter(timePointDays == max(timePointDays)) %>%
  dplyr::top_n(1, wt = estAbund) %>%
  dplyr::ungroup() %>%
  dplyr::select(patient, posid, estAbund, nearestFeature, nearestFeatureDist, STRING_id)

t.wt <- table(d.wt$nearestFeature) 
t.wt[t.wt > 1]

d.Y664F <- dplyr::group_by(data.frame(subset(report$intsites.Y664F, estAbund >= 50)), posid) %>%
  dplyr::filter(timePointDays == max(timePointDays)) %>%
  dplyr::top_n(1, wt = estAbund) %>%
  dplyr::ungroup() %>%
  dplyr::select(patient, posid, estAbund, nearestFeature, nearestFeatureDist, STRING_id)

t.Y664F <- table(d.Y664F$nearestFeature) 
t.Y664F[t.Y664F > 1]
...
report$percent_WT_gt_50 <- sprintf("%.2f%%", (n_distinct(subset(report$intsites.WT, estAbund > 50)$posid) / n_distinct(report$intsites.WT$posid))*100)
report$percent_Y664F_gt_50 <- sprintf("%.2f%%", (n_distinct(subset(report$intsites.Y664F, estAbund > 50)$posid) / n_distinct(report$intsites.Y664F$posid))*100)
...
d <- data.frame(subset(report$intsites.WT, estAbund >= 150))[,c('patient', 'timePoint', 'posid', 'estAbund', 'nearestFeature', 'nearestFeatureDist')]
d$foldChang <- report$RNAseq_WT_D9_vs_KD_D9[match(toupper(d$nearestFeature), toupper(report$RNAseq_WT_D9_vs_KD_D9$gene)),]$log2FoldChange
...
s <- subset(report$intsites.WT, relAbund >= 1)$STRING_id
s <- s[! is.na(s)]
e <- string_db$get_enrichment(s, category = 'KEGG', methodMT = "fdr", iea = FALSE)
#e <- string_db$get_enrichment(s, category = 'Process', methodMT = "fdr", iea = FALSE)
e <- subset(e, pvalue_fdr <= 0.05)
e$geneListLength <- n_distinct(s)
p <- string_db$get_term_proteins(e$term_id)
p <- p[p$STRING_id %in% s,]
p <- dplyr::group_by(p, term_id) %>% dplyr::summarise(genes = paste(unique(preferred_name), collapse = ', ')) %>% dplyr::ungroup()
report$intSites.WT.KEGG <- dplyr::left_join(e, p, by = 'term_id')

report$WT_D9_vs_KD_D9.KEGG$term_description[report$WT_D9_vs_KD_D9.KEGG$term_description %in% report$intSites.WT.KEGG$term_description]
...
# p[which(p$posid == 'chr13+76669931'),]$nearestFeature <- '* AL136441.1'
# p[which(p$posid == 'chr13+76669931'),]$nearestFeatureDist <- 0
# p[which(p$posid == 'chr19+15337500'),]$nearestFeature <- '* BRD4'
# p[which(p$posid == 'chr19+15337500'),]$nearestFeatureDist <- -4955
# p[which(p$posid == 'chr21-14049940'),]$nearestFeature <- '* antisense LIPI'
# p[which(p$posid == 'chr21-14049940'),]$nearestFeatureDist <- 0
# p[which(p$posid == 'chr1+190888946'),]$nearestFeature <- '* AL138927.1'
# p[which(p$posid == 'chr1+190888946'),]$nearestFeatureDist <- 0
# p[which(p$posid == 'chr12+93556319'),]$nearestFeatureDist <- 0
# p[which(p$posid == 'chr4-173327158'),]$nearestFeature <- '* AC097534.1'
# p[which(p$posid == 'chr4-173327158'),]$nearestFeatureDist <- 0
...
report$WT.intSites.KEGG <- createKEGGenrichmentTable(data.frame(report$intsites.WT))
...
s <- report[names(report) %in% c('All', 'WT', 'Y664F', 'LS_WT', 'LS_Y664F')]

invisible(mapply(function(n, x){
  invisible(lapply(c(1, 5, 10), function(x2){
    g <- subset(x, abs(nearestFeatureDist) <= 10000 & relAbund >= x2)
    d <- data.frame(g)
    o <- unlist(lapply(split(d, d$posid), function(x){
      unlist(base::strsplit(x$nearestFeature, ','))
    }))
    
    write(o, file=paste0('data/geneLists/', n, '_relAbund_gte_', x2))
  }))
  
  invisible(lapply(c(5, 10, 50, 100, 500), function(x2){
    g <- subset(x, abs(nearestFeatureDist) <= 10000 & estAbund >= x2)
    d <- data.frame(g)
    o <- unlist(lapply(split(d, d$posid), function(x){
      unlist(base::strsplit(x$nearestFeature, ','))
    }))
    
    write(o, file=paste0('data/geneLists/', n, '_estAbund_gte_', x2))
  }))
}, names(s), s))



# Store a subset of the intSites where all subjects have more than one time point.
report$intSites  <- intSites
report$intSites2 <- subset(intSites, patient %in% report$subjectsWithMultTimePoints)
...
report$TrpM_D9_vs_KD_D9.KEGG <- createKEGGenrichmentTable(subset(d, experiment == 'TrpM_D9_vs_KD_D9' & 
                                                                   ! is.na(log2FoldChange) &  
                                                                   ! is.na(STRING_id) & 
                                                                   padj <= 0.01 & 
                                                                   abs(log2FoldChange) >= 2),
...
d <- RNAseqData[,c('geneName', 'WT_D9_vs_KD_D9', 'WT_D9_vs_KD_D9_padj')]
d <- d[!is.na(d$WT_D9_vs_KD_D9),]
d <- d[abs(d$WT_D9_vs_KD_D9) >= 4,]
d <- d[which(d$WT_D9_vs_KD_D9_padj <= 0.001 | is.na(d$WT_D9_vs_KD_D9_padj)),]
d <- d[order(abs(d$WT_D9_vs_KD_D9), decreasing = TRUE),]
mapped <- string_db$map(data.frame(gene = d$geneName), "gene", removeUnmappedRows = TRUE)
d <- dplyr::left_join(d, mapped, by = c("geneName" = "gene"))
d.up <- subset(d, WT_D9_vs_KD_D9 >= 0 & ! is.na(STRING_id))$STRING_id
d.down <- subset(d, WT_D9_vs_KD_D9 < 0 & ! is.na(STRING_id))$STRING_id

WTvsKD.d9.KEGG <- string_db$get_enrichment(mapped$STRING_id, category = 'KEGG', methodMT = "fdr", iea = FALSE)
WTvsKD.d9.KEGG <- subset(WTvsKD.d9.KEGG, pvalue_fdr <= 0.05)
WTvsKD.d9.KEGG$geneListLength <- n_distinct(mapped$STRING_id)
WTvsKD.d9.GO <- string_db$get_enrichment(mapped$STRING_id, category = 'Process', methodMT = "fdr", iea = FALSE)
WTvsKD.d9.GO <- subset(WTvsKD.d9.GO, pvalue_fdr <= 0.05)
WTvsKD.d9.GO$geneListLength <- n_distinct(mapped$STRING_id)

p <- string_db$get_term_proteins(WTvsKD.d9.KEGG$term_id)
p <- p[p$STRING_id %in% mapped$STRING_id,]
p <- dplyr::rowwise(p) %>% dplyr::mutate(preferred_name2 = ifelse(STRING_id %in% d.up, toupper(preferred_name), tolower(preferred_name))) %>% dplyr::ungroup()
p <- dplyr::group_by(p, term_id) %>% dplyr::summarise(genes = paste0(sort(preferred_name2), collapse = ', ')) %>% dplyr::ungroup()
WTvsKD.d9.KEGG <- dplyr::left_join(WTvsKD.d9.KEGG, p, by = 'term_id')
...
# Update mapped gene table.

string_db <- STRINGdb$new( version="10", species=9606, score_threshold=0, input_directory="/home/everett/project.geneTherapy.NPM-ALK/STRINGdb")
mapped <- string_db$map(data.frame(gene = d$gene), "gene", removeUnmappedRows = FALSE)    # 67% not mapped

i <- which(is.na(mapped$STRING_id));  message('% Unmapped ids: ', round((length(i) / nrow(mapped))*100, 2))
mapped[i,]$STRING_id <- string_db.alt.aliases[match(toupper(mapped[i,]$gene), toupper(string_db.alt.aliases$alias)),]$STRING_id
i <- which(is.na(mapped$STRING_id));  message('% Unmapped ids after update: ', round((length(i) / nrow(mapped))*100, 2))
...
d <- bind_rows(lapply(report[names(report)[grepl('RNAseq', names(report))]], as.data.frame)) %>%
  filter(experiment %in% c('WT_D6_vs_KD_D6', 'WT_D9_vs_KD_D9', 'TrpM_D6_vs_KD_D6', 'TrpM_D9_vs_KD_D9')) %>%
  mutate(abs_log2FoldChange = abs(log2FoldChange)) %>%
  group_by(experiment, gene) %>%
  top_n(n = 1, wt = abs_log2FoldChange) %>%
  ungroup() 



# Retrieve entrez and HGNC ids
ensembl <- useMart("ensembl", dataset = "hsapiens_gene_ensembl")
mapping <- getBM(attributes = c("ensembl_gene_id", "hgnc_symbol", "entrezgene"), mart = ensembl)
d <- dplyr::left_join(d, mapping, by = c("ensembl" = "ensembl_gene_id"))
...
o <- subset(d, experiment = 'WT_D9_vs_KD_D9' & ! is.na(entrezgene))
foldchanges = o$log2FoldChange
names(foldchanges) = o$entrezgene
head(foldchanges)
...
o <- results(ddsTxi, contrast=c("repGrp","WT_D9","KD_D9"))
o[row.names(o) == 'ENSG00000206899.1',]
...
dds <- DESeqDataSetFromTximport(txi, sampleTable, ~ clone + timePoint)

# Search for # RNU6−36P in WT_D9_vs_KD_D9 
design(dds) <- ~ group
dds <- DESeq(dds)
#r <- results(dds, contrast=c("group", "WT_D9", "KD_D9")) 
r <- results(dds, contrast=c("group", "KD_D9", "WT_D9")) 
r[row.names(r) == 'RNU6-36P',]

# Everett result:
#           baseMean log2FoldChange     lfcSE      stat    pvalue      padj
#  RNU6-36P  12.40535       29.96547  5.560388  5.389097    NA          NA

# Tobias result:
# Column # 	Column ID 	Gene name 	ensembl stable ID 	WT_D9_vs_KD_D9 log2FoldChange 	WT_D9_vs_KD_D9 lfcSE 	WT_D9_vs_KD_D9 stat 	WT_D9_vs_KD_D9 padj
# 20617 	ENSG00000206899.1 	RNU6-36P 	ENSG00000206899 	32.6717 	5.77461 	5.65782 	1.94E-07
